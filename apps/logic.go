...
func reviewLogicCert(d uapi.RouteData, resp types.AppResponse, reason string, approve bool) error {
	if approve {
		...
		if err != nil {
			state.Logger.Error("error giving certified bot role to bot, but successfully certified bot: %v", err)
		}
...
func reviewLogicStaff(d uapi.RouteData, resp types.AppResponse, reason string, approve bool) error {
	if approve {
		...
		if strings.HasPrefix(reason, "MANUALLYNOTIFIED ") {
			state.Logger.Info("forcing denial of staff application that was manually notified by a manager", resp.UserID)
			return nil
		}
		...
		if err != nil {
			state.Logger.Error("could not create DM channel with user on denial", resp.UserID, err)
			return errors.New("could not create DM channel with user, please inform them manually, then deny with reason of 'MANUALLYNOTIFIED <your reason here>'")
		}
		...
		if err != nil {
			state.Logger.Error("could not send DM to user on denial", resp.UserID, err)
			return errors.New("could not send DM, please inform them manually, then deny with reason of 'MANUALLYNOTIFIED <your reason here>'")
		}
