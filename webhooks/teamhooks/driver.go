// Teamhooks core driver to manage central processing (webhook retries and pull pending etc.)
package teamhooks
...
func (d Driver) PullPending() *sender.WebhookPullPending {
	return &sender.WebhookPullPending{
		...
		GetEntity: func(id string) (sender.WebhookEntity, error) {
			...
			err := state.Pool.QueryRow(state.Context, "SELECT name FROM teams WHERE id = $1", id).Scan(&name)
